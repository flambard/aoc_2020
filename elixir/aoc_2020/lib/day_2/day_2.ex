defmodule Day2 do
  def count_valid_passwords(input, policy) do
    Enum.count(input, fn row -> policy.(row) end)
  end

  def sled_rental_policy({low, high, policy, password}) do
    [policy_char] = List.Chars.to_charlist(policy)

    policy_matches =
      Enum.count(
        List.Chars.to_charlist(password),
        fn c -> c == policy_char end
      )

    low <= policy_matches and policy_matches <= high
  end

  def toboggan_policy({a, b, policy, password}) do
    match_a = policy == String.at(password, a - 1)
    match_b = policy == String.at(password, b - 1)

    (match_a and not match_b) or
